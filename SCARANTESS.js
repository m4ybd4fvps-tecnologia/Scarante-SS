...
const SPEECH = {
pt: {
start: "Analisando, aguarde o SCARANTE SS terminar",
half: "Scanner em cinquenta por cento. Aguarde mais um pouco.",
probe: "Scanner em noventa por cento. Aguarde mais um pouco.",
done: "SCARANTE SS finalizado. Analise os resultados com cuidado.",
},
en: {
start: "Analyzing, please wait for SCARANTE SS to finish.",
half: "Scanner at fifty percent. Please wait a little longer.",
probe: "Scanner at ninety percent. Almost done.",
done: "SCARANTE SS finished. Analyze the results carefully.",
},
es: {
start: "Analizando, espera que SCARANTE SS termine.",
half: "Escáner al cincuenta por ciento. Espera un poco más.",
probe: "Escáner al noventa por ciento. Ya casi termina.",
done: "SCARANTE SS finalizado. Analiza los resultados con cuidado.",
},
}
...
// Simple regex-based plist parser for basic info
let result = {};
let hashMatch = content.match(/<key>hash<\/key>\s*<string>([^<]+)<\/string>/i);
if (hashMatch) result.hash = hashMatch[1];

let nameMatch = content.match(/<key>name<\/key>\s*<string>([^<]+)<\/string>/i);
if (nameMatch) result.name = nameMatch[1];

let versionMatch = content.match(/<key>CFBundleShortVersionString<\/key>\s*<string>([^<]+)<\/string>/i);
...
return { path, content: parsePlist(content) }
}
} catch(e) {}
return null
}
...
function checkJailbreak(ipsFindings, ipsMeta) {
let jbApps = ["Cydia", "Sileo", "Zebra", "Filza", "Dopamine", "unc0ver", "checkra1n", "palera1n"];
let detected = [];

for (let f of ipsFindings) {
for (let app of jbApps) {
if (f.bundle.toLowerCase().includes(app.toLowerCase()) || (f.name && f.name.toLowerCase().includes(app.toLowerCase()))) {
detected.push(app);
}
}
}
...
detected.push("Certificados Raiz (Possível Proxy/JB)");
}

return detected.length > 0 ? detected.join(", ") : "Não detectado";
}
...
function buildHTML(findings, netEntries, cheatAppFindings, knownCheatFindings, ipsFindings, ipsMeta, _unused, ghostAppFindings, proxyLoginFindings, filename, plistInfo) {
let jbStatus = checkJailbreak(ipsFindings, ipsMeta);
...
let uptimeBg = uptimeWarning ? "background:linear-gradient(90deg,#2a1000,#1a0800)" : "background:#1a1a1a"
...
body { background:#000000; color:#ffffff; font-family:-apple-system,ui-monospace,monospace; font-size:13px; }

/* HERO */
.hero { background:#000000; border-bottom:1px solid #ff0000;
background: linear-gradient(160deg, #1a0000 0%, #000000 70%);
...
background:radial-gradient(circle, #ff00000d 0%, transparent 70%);
border-radius:50%; pointer-events:none;
}
.hero-eyebrow {
font-size:9px; letter-spacing:3px; color:#ff000055;
...
.hero-credits {
font-size:11px; color:#ff3b30; letter-spacing:2.5px;
...
.hero-name span { color:#ff0000; }
.hero-file {
font-size:10px; color:#556; word-break:break-all;
padding:7px 10px; background:#121212;
border-radius:7px; border-left:3px solid #ff000033;
margin-bottom:14px; line-height:1.5;
text-align:left;
}
.hero-file strong { color:#ff000099; }
.hero-grid { display:grid; grid-template-columns:1fr 1fr; gap:8px; }
.hg-card { background:#1a1a1a; border:1px solid #330000;
background:#121212; border-radius:8px;
padding:9px 12px; border:1px solid #333333;
}
.hg-label { font-size:9px; color:#8b0000; letter-spacing:1px; text-transform:uppercase; margin-bottom:3px; }
.hg-val { font-size:12px; color:#ddd; }
.hg-val.cyan { color:#ff0000; font-weight:bold; font-size:14px; }
.hg-val.warn { color:#ff8800; font-weight:bold; font-size:13px; }
.hg-card-warn { background:#fff5e6 !important; border-color:#ff880055 !important; }
...
.lang-btn {
background:#121212; border:1px solid #333333; border-radius:20px;
color:#556; font-size:10px; letter-spacing:1px; padding:4px 10px;
cursor:pointer; font-family:inherit; transition:all 0.2s;
text-transform:uppercase; font-weight:600;
}
.lang-btn:hover { border-color:#ff000055; color:#ff0000aa; }
.lang-btn.active { background:#200000; border-color:#ff0000; color:#ff0000; }
...
.uptime-text { font-size:11px; color:#aaa; }
...
.stat { background:#1a1a1a; border:1px solid #330000;
flex:1; background:#121212; border-radius:10px;
padding:12px 6px; text-align:center; border:1px solid #333333;
}
.stat .num { font-size:28px; font-weight:bold; line-height:1; }
.stat .lbl { font-size:9px; color:#8b0000; margin-top:4px; letter-spacing:1px; text-transform:uppercase; }
...
.section-header .sh-sub { font-size:10px; color:#8b0000; margin-top:1px; }
...
.divider { height:1px; background:#300000; margin:20px 0; }

/* CARDS */
.card { background:#1a1a1a; border:1px solid #330000;
background:#121212; border-radius:12px;
margin-bottom:12px; overflow:hidden;
border:1px solid #333333; border-left:4px solid #ddd;
}
...
.conns { font-size:10px; color:#8b0000; }
.card-domain {
...
border-top:1px solid #300000; align-items:flex-start;
}
.label { color:#8b0000; min-width:65px; font-size:10px; padding-top:1px; flex-shrink:0; line-height:1.4; }
.sub { color:#660000; font-size:9px; }
.val { color:#ccc; word-break:break-all; flex:1; font-size:11px; line-height:1.5; }
.isp { color:#ffbb00; }
.reason { color:#ff8a80; }
...
border:1px solid #6600004aa55; border-radius:12px;
...
.ghost-title { font-size:12px; font-weight:bold; color:#aaa9ff; letter-spacing:0.5px; }
.ghost-sub { font-size:10px; color:#660000466; margin-top:2px; }
.ghost-count { background:#0a0a2a; color:#6677ee; font-size:11px; font-weight:bold; padding:2px 8px; border-radius:10px; border:1px solid #6600004aa55; align-self:flex-start; }
...
.ghost-label { font-size:9px; color:#660000; background:#0a0a1a; padding:2px 6px; border-radius:6px; border:1px solid #223; }
.ghost-hint { font-size:9px; color:#660000466; border-top:1px solid #1a1a33; padding-top:8px; }
...
.ips-row-warning { background:#fff5e6 !important; border-color:#ff880033 !important; }
...
background:#fdf2ff; border:1px solid #330033; border-radius:8px; padding:8px 10px;
}
...
.ff-tag { font-size:9px; color:#8b0000; min-width:100px; text-transform:uppercase; letter-spacing:0.5px; }
...
.ff-session-num { font-size:9px; color:#8b0000; text-transform:uppercase; letter-spacing:0.5px; }
...
.http-off { color:#bbb; font-weight:bold; }
.http-banner { color:#ff00cc; font-weight:bold; text-transform:uppercase; font-size:10px; }
.none { color:#660000; }
.bundle {
display:inline-block; background:#121212; border-radius:5px;
padding:2px 6px; font-size:9px; color:#556; margin:1px;
word-break:break-all; border:1px solid #333333;
}
.domain-row { padding:3px 0; font-size:11px; color:#ccc; word-break:break-all; }
.domain-badge {
display:inline-block; font-size:9px; font-weight:bold;
...
.ok {
background:#e6ffed; border:1px solid #1a3020; color:#4caf50;
...
<div class="hero-name">SOUZA<span>SS</span></div>
<div class="hero-credits"><span class="credit-name">SOUZA SS</span> &middot; <span class="credit-name">Samir</span> &middot; <span class="credit-name">Katiau</span></div>
<div class="lang-bar">
...
${plistInfo ? `<div class="hero-file" style="border-left-color:#ff0000"><strong>Plist Hash:</strong> ${plistInfo.hash || "N/A"}<br><strong>Nome:</strong> ${plistInfo.name || "N/A"}</div>` : ""}
...
<div class="hg-card">
<div class="hg-label">Modelo iPhone</div>
<div class="hg-val cyan">${ipsMeta.model || "Desconhecido"}</div>
</div>
<div class="hg-card">
<div class="hg-label">Jailbreak</div>
<div class="hg-val ${jbStatus !== "Não detectado" ? "warn" : "cyan"}">${jbStatus}</div>
</div>
...
ipsMeta.model = parsed.header.model || "iPhone"
}
...
let plistInfo = null
let notice3 = new Alert()
notice3.title = "Arquivo .plist (Opcional)"
notice3.message = "Deseja selecionar um arquivo .plist para verificar Hash e Nome?"
notice3.addAction("Selecionar .plist")
notice3.addCancelAction("Pular")
if (await notice3.present() !== -1) {
let res = await readPlistFile()
if (res) plistInfo = res.content
}
...
let html = buildHTML(findings, netEntries, cheatAppFindings, knownCheatFindings, ipsFindings, ipsMeta, [], ghostAppFindings, proxyLoginFindings, filename, plistInfo)
